...
if(isset($_SESSION['PNSDOTVN_CART']) && !empty($_SESSION['PNSDOTVN_CART']) && count($_SESSION['PNSDOTVN_CART']) > 0)
{
	...
	$msg = $pns->buildOrderMsg($oid, $def, $_LNG, $lang);
	if(!empty($msg) && is_object($msg))
	{	
		$m = $pns->getMemberInfo(MEMBER_SYS && isset($_SESSION['member']['id']) ? $_SESSION['member']['id'] : $_SESSION['PNSDOTVN_CHECKOUT_CSID']);
		...
		$mail->AddCC('vattu@pns.vn', 'chauruachen.pns.vn');
		//$mail->AddBCC('vuongnt777@gmail.com', 'chauruachen.pns.vn');
