...
$.Autocompleter = function(input, options) {
	...
	function selectCurrent() {
		...
		/* Set vào biến toàn cục để phục vụ sự kiện click */		
	...
	function reset_iCat(value){
		/* Nếu iCat <= 0 gán lại biến hidden = 0 và disable đi  */
		if (value <= 0){
			$("#header_bar_category_id").attr('disabled','disabled'); 
			$("#header_bar_category_id").val('0'); 
		}
		/* Nếu khác 0 thì gán lại biến hidden và enable lên */
		else{
...
$.Autocompleter.Select = function (options, input, select, config) {
	...
	function init() {
		...
		list = $("<ul/>").appendTo(element).mouseover( function(event) {
			...
			//Left click redirect đến quicksearch
			if(event.which == 0 || event.which == 1){
				//if (autocomplete_selected_category_id == 0) window.location.href = "/" + autocomplete_selected_keyword;
				...
				window.location.href = "/" + autocomplete_selected_category_id;
			}
			//return false;
		}).mousedown(function() {
	...
	function fillList() {
		...
		for (var i=0; i < max; i++) {
			...
			//Chèn quảng cáo, record đầu tiên dành cho qcáo
			if (i==0){
				if (!ads_element)	ads_element = $("<div/>").html(data[i].value).prependTo(element);
				else ads_element.html(data[i].value);
			}
			//Nếu i > 0 sẽ là content
			if (i > 0){
