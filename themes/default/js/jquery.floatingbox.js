...
{this.targetX=-250;this.targetY=10;this.interval=20;this.menu=box;this.index=index;this.options=options;this.hasInner=typeof(window.innerWidth)=='number';this.hasElement=document.documentElement&&document.documentElement.clientWidth;this.move=function()
...
{var stepX,stepY;this.computeShifts();stepX=(this.shiftX+this.targetX-this.nextX)*.07;if(Math.abs(stepX)<.5)
{stepX=this.shiftX+this.targetX-this.nextX;}
stepY=(this.shiftY+this.targetY-this.nextY)*.07;if(Math.abs(stepY)<.5)
...
{this.computeShifts();this.nextX=this.shiftX+this.targetX;this.nextY=this.shiftY+this.targetY;this.move();};this.position=function(targetX,targetY)
...
{case'top':this.targetY=5;break;case'bottom':this.targetY=-((jQuery(this.menu).outerHeight()+5));break;case'left':var wscreen=screen.availWidth;var wpage=1000;var left=parseInt((wscreen-wpage)/2-153-10);this.targetX=left;break;case'right':var wscreen=screen.availWidth;var wpage=1000;var right=parseInt(wscreen-((wscreen-wpage)/2)-5);this.targetX=right;break;case'middle':this.targetY=100;break;}}
