...
$utls = new Utilities();
$col=array('id' => 'ID', 'name' => 'Tên màu', 'picture' => 'Hình', 'ordering' => 'Thứ tự', 'created' => 'Ngày tạo', 'modified' => 'Ngày sửa', 'status' => 'Trạng thái');
...
if($isDelete)
{
	...
	$affect  = $dbf->deleteDynamic(prefixTable . 'color', 'id in (' . $arrayid . ')');
	if($affect > 0) 
	{
		$dbf->deleteDynamic(prefixTable . 'color_desc', 'id in (' . $arrayid . ')');			
...
if($subInsert)
{
	...
	if(isset($_POST['name']) && !empty($_POST['name']))
	{
		...
		$affect = $dbf->insertTable(prefixTable . 'color', $data);
		if($affect > 0) 
		{
			$_id = $affect;			
	  		...
	  		foreach($langs as $lang)
			{
				...
				$dbf->insertTable(prefixTable . 'color_desc', $value[$lang]);
			}		
			$code = $value['vi-VN']['rewrite'] . '-' . $_id;
			$dbf->updateTable(prefixTable . 'color', array('code' => $code), 'id = ' . $_id);		
...
if($subUpdate)
{
	...
	if(isset($_GET['edit']) && !empty($_GET['edit']) && isset($_POST['name']) && !empty($_POST['name']))
	{
		...
		$affect = $dbf->updateTable(prefixTable . 'color', $data, 'id = ' . $_id);
		if($affect > 0)
		{
			$langs = array_keys($value);
	  		foreach($langs as $lang)
			{
				$dbf->updateTable(prefixTable . 'color_desc', $value[$lang], 'lang = "' . $lang . '" and id = ' . $_id);			
			}			
			$code = $value['vi-VN']['rewrite'] . '-' . $_id;
			$dbf->updateTable(prefixTable . 'color', array('code' => $code), 'id = ' . $_id);
...
if($isEdit)
{
  	$rst = $dbf->getDynamic(prefixTable . 'color', 'id = ' . (int) $_GET['edit'], '');
  	if($rst)
  	{
		...
		$vi = $dbf->getArray(prefixTable . 'color_desc', 'id = "' . $id . '" and lang = "vi-VN"', '', 'stdObject');
		#$en = $dbf->getArray(prefixTable . 'color_desc', 'id = "' . $id . '" and lang = "en-US"', '', 'stdObject');
  	}
} ?>
...
<?php if(!$isEdit && !$isInsert) {
	...
	$url = 'color.php?';
    $mang = $dbf->paging(prefixTable . 'color', '1=1', 'ordering, id', $url, $PageNo, $PageSize, $Pagenumber, $ModePaging);
	echo $dbf->panelAction($mang[1]) ?>
<!-- view -->
<div id="panelView" class="panelView"><?php $dbf->normalView($col, 'color.php', $mang, $statusAction, '&caturl=' . $iscatURL, $msg = '') ?></div>
<!-- end view-->
<?php } ?>
