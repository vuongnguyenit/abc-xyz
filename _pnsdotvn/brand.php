...
$utls = new Utilities();
$col=array('id' => 'ID', 'name' => 'Tên thương hiệu', 'picture' => 'Hình', 'ordering' => 'Thứ tự', 'created' => 'Ngày tạo', 'modified' => 'Ngày sửa', 'status' => 'Trạng thái');
$iscatURL = isset($_GET['caturl']) && !empty($_GET['caturl']) ? (int) $_GET['caturl'] : 0;
$titleMenu = 'QUẢN LÝ DANH MỤC THƯƠNG HIỆU';
#$brand_rewrite = array('vi-VN' => 'thuong-hieu', 'en-US' => 'brand');
$brand_rewrite = array('vi-VN' => 'thuong-hieu');

if($isDelete)
{
  	$arrayid = substr($_POST['arrayid'], 0, -1);
  	$id_array = explode(',', $arrayid);  
	$affect  = $dbf->deleteDynamic(prefixTable . 'brand', 'id in (' . $arrayid . ')');
	if($affect > 0)
	{
		$dbf->deleteDynamic(prefixTable . 'brand_desc', 'id in (' . $arrayid . ')');
		$dbf->deleteDynamic(prefixTable. 'url_alias', 'route = "brand" and type = "detail" and related_id in (' . $arrayid . ')');  
...
if($subInsert)
{
	...
	if(isset($_POST['name']) && !empty($_POST['name']))
	{
		...
		$affect = $dbf->insertTable(prefixTable . 'brand', $data);
		if($affect > 0)
		{			
			$_id = $affect;			
	  		...
	  		foreach($langs as $lang)
			{
				...
				$dbf->insertTable(prefixTable . 'brand_desc', $value[$lang]);
				
				#$_url_alias = strtolower(substr($lang, 0, -3) . '/' . $brand_rewrite[$lang] . '/' . $_id . '-' . $value[$lang]['rewrite']) . EXT;	
				$_url_alias = strtolower(substr($lang, 0, -3) . '/' . $brand_rewrite[$lang] . '/' . $value[$lang]['rewrite'] . '-' . $_id) . EXT;				
				$dbf->insertTable(prefixTable . 'url_alias', array('url_alias' => $_url_alias, 'route' => 'brand', 'type' => 'detail', 'related_id' => $affect, 'lang' => $lang));
			}			
...
if($subUpdate)
{
	...
	if(isset($_GET['edit']) && !empty($_GET['edit']) && isset($_POST['name']) && !empty($_POST['name']))
	{
		...
		$affect = $dbf->updateTable(prefixTable . 'brand', $data, 'id = ' . $_id);
		if($affect > 0)
		{			
			$langs = array_keys($value);
	  		foreach($langs as $lang)
			{
				$dbf->updateTable(prefixTable . 'brand_desc', $value[$lang], 'lang = "' . $lang . '" and id = ' . $_id);
				
				#$_url_alias = strtolower(substr($lang, 0, -3) . '/' . $brand_rewrite[$lang] . '/' . $_id . '-' . $value[$lang]['rewrite']) . EXT;
				$_url_alias = strtolower(substr($lang, 0, -3) . '/' . $brand_rewrite[$lang] . '/' . $value[$lang]['rewrite'] . '-' . $_id) . EXT;
				$dbf->updateTable(prefixTable . 'url_alias', array('url_alias' => $_url_alias), 'route = "brand" and type = "detail" and lang = "' . $lang . '" and related_id = ' . $_id);
			}
...
if($isEdit)
{
  	$rst = $dbf->getDynamic(prefixTable . 'brand', 'id = ' . (int) $_GET['edit'], '');
  	if($rst)
  	{
		...
		$vi = $dbf->getArray(prefixTable . 'brand_desc', 'id = "' . $id . '" and lang = "vi-VN"', '', 'stdObject');
		#$en = $dbf->getArray(prefixTable . 'brand_desc', 'id = "' . $id . '" and lang = "en-US"', '', 'stdObject');
  	}
} ?>
...
<div id="panelForm" class="panelForm">
  <table id="mainTable" cellpadding="0" cellspacing="0">
    ...
    <tr>
      <td class="boxGrey" colspan="2">Mô tả thương hiệu <!--[Tiếng Việt]--></td>
    </tr>
    <tr>
...
<?php if(!$isEdit && !$isInsert) {
	...
	$url = 'brand.php?';
    $mang = $dbf->paging(prefixTable . 'brand', '1=1', 'ordering, id', $url, $PageNo, $PageSize, $Pagenumber, $ModePaging);
	echo $dbf->panelAction($mang[1]) ?>
<!-- view -->
<div id="panelView" class="panelView"><?php $dbf->normalView($col, 'brand.php', $mang, $statusAction, '&caturl=' . $iscatURL, $msg = '') ?></div>
<!-- end view-->
<?php } ?>
