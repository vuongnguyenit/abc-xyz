...
define('pns_PATH_ISWIN', (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN'));
/** boolean True if a Mac based host */
define('pns_PATH_ISMAC', (strtoupper(substr(PHP_OS, 0, 3)) === 'MAC'));

define('ROOT_PATH', '');
define('pns_PATH_ROOT', '');
...
class pns_Path
{
		...
		if (is_dir($path))
		{
			...
			while ($file = readdir($dh))
			{
				if ($file != '.' && $file != '..') {
					$fullpath = $path.'/'.$file;
					if (is_dir($fullpath)) {
						if (!pns_Path::setPermissions($fullpath, $filemode, $foldermode)) {
							$ret = false;
						}
					} else {
						if (isset ($filemode)) {
	...
	function check($path)
	{
		...
		if (strpos($path, '..') !== false) {
			$vnT->error[] = 'pns_Path::check Use of relative paths not permitted'; // don't translate
			exit();
		}
		$path = pns_Path::clean($path);
		if (strpos($path, pns_Path::clean(pns_PATH_ROOT)) !== 0) {
			$vnT->error[] = 'pns_Path::check Snooping out of bounds @ '.$path; // don't translate
			exit();
		}
	}
