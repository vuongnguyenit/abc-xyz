...
class DBFunction extends HTML{
	...
	function getDynamicJoin($tbName1,$tbName2,$arrayNewName=null,$typejoin="inner join",$condition,$orderby,$on, $debug = false){
		try{
				...
				$str="";
				foreach($arrayNewName as $key=>$value) $str.=" t2.$key as $value ,";
		...
		}catch(Exception $ex)
		{
			...
			function getArray($tbName,$condition,$orderby,$mode="",&$array_col=null) {
				try
                {
					...
					switch($mode)
					{
    					 case "stdObject": while($row=$this->nextObject($rst))$array_row[]=$row;
    					   break;
                         case "Assoc": while($row=$this->nextAssoc($rst))$array_row[]=$row;
    					   break;
                         case "Row": while($row=$this->nextRow($rst))$array_row[]=$row;
    					    break;
						 case "Array": while($row=$this->nextData($rst))$array_row[]=$row;
    					    break;
    					 default: while($row=$this->nextData($rst))$array_row[]=$row;
    					   break;
					}
                    unset($rst,$str,$row);
				    return $array_row;
	...
	function SelectWithRowArray($array, $match = '', $idName, $datatextfield, $datavaluefield, $arrayMode = null, $Options = null)
	{
		try
		{
			...
			if(isset($Options['firstText']) && !empty($Options['firstText'])) $str .= '<option value="">' . $Options['firstText'] . '</option>';
				if(count($array) > 0)
				{
                	switch($arrayMode)
                    {
				...
				try {
						...
						if(!empty($Options) AND count($Options)>0)
							foreach($Options as $name => $value)
								if($name=='firstText') $firstText = $value;
								else $att .= ' ' . $name . '="' . $value . '"';
						$str='<select name="'.$idName.'" id="'.$idName.'"' . $att . '>';
						if(!empty($Options["firstText"])) $str.='<option value="">'.$firstText.'</option>';
						if(count($array)>0)
                        foreach($array as $key => $value) $str.='<option value="'.$key.'"'.(($match==$key)?" selected ":"").'>'.$value.'</option>';
						$str.='</select>';
                        unset($key,$value);
						return $str;
				...
				try {
					...
					if(!empty($arrayOption) AND count($arrayOption)>0)
						foreach($arrayOption as $name => $value)
							if($name=='firstText') $firstText = $value;
							else if($name=='char') $char = $value;
							else $att .= ' ' . $name . '="' . $value . '"';
					$str = "<select id=\"" . $idName . "\" name=\"" . $idName . "\"" . $att . ">";
					if($firstText!='') $str .= "<option value=\"\">". $firstText . "</option>";
